...
describe('Login screen', () => {
    ...
    it('should create survey', () => {
        ...
        cy.contains('Scope of Work Option', { timeout: 10000 }).should('be.visible');

        cy.contains('button', 'Enter Scope of Work Manually').click();
        cy.contains('button', 'Add Scope of Work Element').click();

        cy.contains('h6', 'Double click to edit', { timeout: 10000 }).should('be.visible');

        cy.contains('button', 'Go to Context').click();

        cy.contains('button', 'Background', { timeout: 10000 }).should('be.visible');
        ...
        cy.contains('span', 'Section name', { timeout: 40000 }).should('be.visible');
